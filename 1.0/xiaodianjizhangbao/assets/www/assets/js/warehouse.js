    ...
    function renderProducts(products){
        ...
        $.each(products, function (j, data){
            if(data.p_pic.indexOf("attachments") !== -1){
                data.p_pic = "http://10.0.2.2/"+data.p_pic;
            }

            if(data.p_pic.indexOf("base64") < 0 && data.p_pic.indexOf("attachments") < 0){
                var user = JSON.parse(localStorage.getItem("user"));
                data.p_pic = "http://10.0.2.2/attachments/"+user.attachmentsDir + "/thumb_"+data.p_pic;
            }
            ...
            tem += "<footer class=\"flexBox\"><a href=\"edit_product.html?id={p_id}&pageNum={pageNum}&type={parentType}\" class=\"J-edit box J-hideTabBar\" target='_blank' data-id=\"{p_id}\">修改</a><a href=\"javascript:void(0)\" class=\"J-del box\" data-id=\"{p_id}\" data-name=\"{p_name}\" style=\"background:red;color:#fff;\">删除</a></footer>";
            tem += '</li>';
