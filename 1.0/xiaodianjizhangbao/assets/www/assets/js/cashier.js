...
define(function (require, exports, module) {
    ...
    if(localStorage.getItem("warehouseIsThere") && localStorage.getItem("warehouseIsThere") == 'false'){
        $('#J-tip').html('您的仓库里还没有商品哦，现在就<a href="add.html" target="_blank" title="添加商品" class="J-hideTabBar">添加</a>').show();
    }
            ...
            on: {
                success: function (data){
                    if(data.data){
                        ...
                        data = {data:{tip_rent: "off"}};
                    }
                    if(!data.data.tip_rent){
                        data.data.tip_rent = "off";
                    }
                    if(data.data.tip_rent === 'on'){
                        var today = new Date();
...
//    });
    ...
    function addPrice() {
        ...
        if (!count) {
            ...
            return Utils.alert('当前销售数量为'+count+'个，不能增加其它价格！');
        }
        ...
        if(countSplit&&countSplit[1]){
            denominator =Math.pow(10, countSplit[1].length);
        };
        var options ='';
        for (
            var i=(1/denominator);
            i <count +(1/denominator);
            (countSplit[1] ?i=(i*1+1/denominator).toFixed(countSplit[1].length) : i++)
            ){
            var selected ='';
            if(countSplit[1]){
                if(i*1=== (count - selectedCount).toFixed(countSplit[1].length)*1){
                    selected = "selected";
                }
            }else{
                if(i*1=== (count - selectedCount)) {
                    selected = "selected";
                }
            }
    ...
    function getCount() {
        ...
        if (!count) {
            ...
            Utils.alert("销售数量不能为0");
            return false;
        }else if(count.match(/\./g) && count.match(/\./g).length >= 2){
            Utils.alert("销售数量不正确");
            return false;
        }
//        else if (!/^\d+$/.test(count)) {
//            Utils.alert("销售数量必须是整数");
//            return false;
//        }
        count = count * 1;
        return count;
    ...
    function queryProducts(byMoreBtn) {
        ...
        if (data) {
            if(queryProductsIO && queryProductsIO.ajaxObj){
    ...
    function noData(data) {
        tipBox.html('没有您要的商品，现在就<a href="add.html" target="_blank" title="添加商品" class="J-hideTabBar">添加</a>').show();
        productList.hide();
    }
    ...
    function renderProducts(data) {
        ...
        $.each(data, function (i, d) {
            if(d.p_pic.indexOf("attachments") !== -1){
                d.p_pic = "http://10.0.2.2/"+d.p_pic;
            }

            if(d.p_pic.indexOf("base64") < 0 && d.p_pic.indexOf("attachments") < 0){
                var user = JSON.parse(localStorage.getItem("user"));
                d.p_pic = "http://10.0.2.2/attachments/"+user.attachmentsDir + "/thumb_"+d.p_pic;
            }
    ...
    function bindUItoPlist() {
        ...
        productList.find("li").unbind().bind("click", function () {
            ...
            gotoCashier();
            queryProductsIO && queryProductsIO.ajaxObj.abort();
    ...
    function selling() {
        tradeData = {};
        if (validate()) {
            Utils.confirm("确认记账？", function (which) {
                    if (getCount() > ($.trim($('#J-cashierProductPreview .J-count').html()) * 1)) {
                        Utils.tip("销售数量大于当前商品的库存！", 2000);
                        setTimeout(function (){
                            new IO({
                                url: "cashier.php",
    ...
    function validate() {
        ...
        if (result && pricesNode.length >= 1) {
            ...
            $.each(pricesNode, function (i, pn) {
                ...
                totalCount += count;
                if (!price) {
                    Utils.alert("第 " + (i + 1) + " 个销售价格不能为空");
                    result = false;
                    return false;
                } else if (!/^\d+\.?\d{0,2}$/.test(price)) {
                    result = false;
                    Utils.alert("第 " + (i + 1) + " 个销售价格不正确");
                    return false;
                } else if (!count) {
                    Utils.alert("第 " + (i + 1) + " 个销售数量不能为空");
                    result = false;
                    return false;
                } else if ((count <= 0)/* || (!/^\d+$/.test(count))*/) {
                    result = false;
                    Utils.alert("第 " + (i + 1) + " 个销售数量不正确");
                    return false;
                }
            });

            var totalCountSplit = (getCount()+'').split('.');
            if(totalCountSplit && totalCountSplit[1]){
                totalCount = totalCount.toFixed(totalCountSplit[1].length)*1;
            }

            if (result && (totalCount !== getCount())) {
                result = false;
                if (pricesNode.length > 1) {
                    Utils.alert("" + pricesNode.length + "个销售价格的销售数量和与总销售数量不相等");
                } else {
                    Utils.alert("销售价格对应的销售数量与总销售数量不匹配");
                }
            }
            $.each(pricesNode, function (i, pn) {
                var price = $.trim($(pn).find("input[type=number]").val()) * 1;
