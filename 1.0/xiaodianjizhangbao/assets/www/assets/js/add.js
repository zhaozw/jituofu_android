...
define(function (require, exports, module){
    ...
    function addType(){
        Utils.prompt('请输入分类名称', function (typename){
            typename = $.trim(typename);
            if(typename){
                $('#J-addType').unbind().html("处理中...");
                addTypeIO = type.add(typename, function (data){
...
//    });
    ...
    function sendAddRequest(){
        ...
        if(!$.trim(photo.val()) && (uploadStatus.find(".loading").css("display") === "none" && uploadStatus.find(".error").css("display") === "none")){
            ...
            Utils.alert('单价不能为空');
            return false;
        }else if($.trim(pPriceNode.val()) && $.trim(pPriceNode.val())*1 == 0){
            window.scrollTo(0, pPriceNode.offset().top);
            Utils.alert('单价不能为0');
            return false;
        ...
        new IO({
            ...
            on: {
                success: function (data){
                    ...
                    Utils.confirm(""+$.trim(pNameNode.val())+" 入库成功，可以记账啦 :-)", function (which){
                ...
                error: function (data){
                    ...
                    Utils.confirm("入库失败", function (which){
                        sendAddRequest();
                    }, function (){
                        $('#J-addPropduct-btn').unbind().click(function (e){
                            e.preventDefault();
